...
To get a better overview how the item buy costs and profits relate to each other, we'll do a quick cluster analysis.
...
# We could sclae but won't get the real values afterwards
...
df_cl %>% 
  ggplot(aes(unit_price_gold_buys, profit)) +
  geom_point() +
  geom_text(aes(label = id), size = 3, check_overlap = FALSE, just = 0, nudge_y = 0.1) +
  ylab("Profit") +
  xlab("Cost Buys") +
...
We can see that profit is not a question of buy costs, there are items with high profit for low and for high costs.
...
After testing, the prediction is better when we take only `unit_price_gold_buys` feature into account in opposite to taking more into account like `type`, `rarity` or `level`. 
...
We now have one predictor and one outcome. The other fields are for reference if we want to dig deeper into the results of the model.

The recipe creates dummy variables for all nominal predictors, this is useful for `type` and `rarity` but as we kicked those out, it doesn't have any effects.

Same is true for zv which removes n/a vectors which is helpful for `item_type` and `item_weight_class` as these values are not present for all rows, but we don't use these features.
...
We use the `random forest` regression model to train with the `ranger` engine.
...
```
Well bad luck, nothing to gain here.


Nonetheless, the error values are already pretty low.
If we compare predictions with true values, we can see that the model is surprisingly accurate. 

The lasso regression (using the `glm` engine) works even better than the random forest model (using the `ranger` engine).

```{r visualize_model }

sells_join_aug <- sells_aug %>% 
