...
9999 gold seems to be the maximum price of an item.
...
item_list_sells %>%
  ...
  gt(rowname_col = "name") %>%
  tab_header(title = "Items being sold", subtitle = "Mean costs and sum of occurrences") %>%
  fmt_number(
    columns = mean_price,
    suffixing = "G"
  ) %>% 
  fmt_number(
    columns = sum_quantity,
    decimals = 0,
    use_seps = TRUE
  ) %>% 
  fmt_markdown(
  ...
  summary_rows(
   ...
   tab_footnote(
    ...
    locations = cells_column_labels(columns = mean_price)
  ) %>%
  tab_source_note(
