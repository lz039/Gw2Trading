...
When selling items, 5% of the cost is a fee for the auction house.
When buying items, also 5% of the price goes to the bank.
...
9999 gold seems to be the maximum possible price of an auction.
Our top item here, the `Festive Grymm Svaard`, is a cosmetic item that can only be bought with real money and lot of luck.
...
unit_price_sells <- item_list_sells %>%
  mutate(quantity_all = listings * quantity)
  mutate(unit_price_gold_sells = unit_price / 10000)
...
When buying and selling the item **`r exp_sold_item_name`** in category **CraftingMaterial** with the highest **mean price** on sell, the earning is **`r win`** per unit. 

There are currently **`r requested`** requested items and **`r available`** available.

This may be a profit of **`r profit` gold** (after taxes).
This would initially cost us **`r init_costs` gold**.

If we would have more of this item, we could even make **`r format(profit_possible, scientific=F)` gold**.
