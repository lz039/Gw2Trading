...
1. Difference between `buy` and `sell prices`?
2. What is the `return of investment (ROI)`?
...
Looks good! Because there is no smaller unit than copper, it's fine to have it as integer.
...
We got a lot of data now, let's strip it down a bit. We need to make some assumptions now:

- Only take 3000 most profit items that also have at least 10 more sells than buys.
...
df_top_profit <- df_all %>% 
    ...
    summarise(profit = round(min(profit), digits = 2),
              id = unique(id),
              mean_more_sells = mean(more_sells)) %>% 
    ungroup() %>% 
    arrange(desc(profit)) %>%
    filter(mean_more_sells > 10) %>% 
    head(3000) %>% 
    select(id)

df_all_top_profit <- df_all %>% 
  filter(id %in% df_top_profit$id)
...
df_top_profit_sells_mean <- df_all_top_profit %>% 
  group_by(name) %>%
  summarize(id = unique(id),
            mean_unit_price_gold_sells = mean(unit_price_gold_sells), 
            mean_estimation_window_sells = mean(estimation_window_sells))

df_top_profit_sells_mean %>% 
  ggplot(aes(mean_unit_price_gold_sells, 
             mean_estimation_window_sells, 
             label = id)) +
  geom_point() +
  geom_text(size = 3,
            check_overlap = FALSE,
...
```{r remove_unused_vars}
rm(highest_order)
rm(highest_offer)
```


#### [WIP] Price distribution
...
- The "(3) teal" cluster are mid-cost, high profit items but there are only three of them in the cluster. (should buy with enough money)
- The "(4) purple" cluster are mid-cost, low profit items (don't buys)

It's interesting that there are a few items in the "(2) green" cluster that are also lower on profit and would probably also fit in the "(1) red" cluster, but 
it seems that the "(2) green" cluster is also characterized by a higher price than the "(1) red" one.
...
- Filter for types: `r relevant_types$type`
- Filter for rarity: `r relevant_rarities`
